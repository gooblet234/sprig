...
static void upl_flush_buf(void) {
  ...
  printf("i wrote an awesome story on page (%d/%lu)\n",
         upl_state.page,
...
static int upl_stdin_read(void) {
  ...
  for (;;) {
    ...
    if (c == PICO_ERROR_TIMEOUT) {
        puts("bro ts is taking too long i have stuff to do");
        return 0;
    }
      ...
      case UplProg_Header: {
          ...
          if (upl_state.name_i < sizeof(upl_state.name) / sizeof(char)) // read game
          {
              ...
              if (upl_state.len_i >= sizeof(uint32_t)) {
                  printf("im not reading %lu chars\n", upl_state.len);
                  ...
                  puts("chainsaw devil ate games bro");
              }
          }
      } break;
      case UplProg_Body: {
        ...
        if (upl_state.len_i % FLASH_PAGE_SIZE == 0) {
          puts("inserting poo key to elect yo floop");
          upl_flush_buf();
        }
          ...
          puts("ALL_GOOD :DD :D :D :DD :D :D :D 8==D");
          memset(&upl_state, 0, sizeof(upl_state));
