...
JERRYXX_FUN(setMap) {
  dbg("aight mofos we settin a map | hi im xenon and i am apologizing on behalf of john sprig for his crude words.");
  
  dbg("module_native::setMap");
  JERRYXX_CHECK_ARG(0, "str");

  dbg("thats a nice string you got there");
...
#define jerry_create_error_sprite_freed() \
  jerry_create_error(JERRY_ERROR_COMMON, \
     (jerry_char_t *)"Why isnt the sprite on the map anymore")
