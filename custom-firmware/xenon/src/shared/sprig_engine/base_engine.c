...
static void runMachineCode(uint8_t *bytes, uint32_t len) {
	...
	void (*fn)(void) = (void (*)(void))((uintptr_t)code | 1);
	...
	jerry_heap_free(code, padded_len);
}
...
static void map_set(char *str) {
  ...
  if (state->map == NULL) {
    yell("WTF WHY IS IT SO BIG HOW DID YOU DO THIS");
    snprintf(errorbuf, sizeof(errorbuf), "bro are you dumb the map is way too large to fit in the memory (%dx%d)", state->width, state->height);
    fatal_error();
  }

  dbg("hey i got a map, ig im gonna allocate the sprites");
    ...
    switch (*str) {
      ...
      default: {
        ...
        *s = (Sprite) { .x = tx, .y = ty, .kind = *str };
        dbg("bro im so good at filling they call me the filler because i filled in some fields");

        state->map[tx + ty * state->width] = s - state->sprite_pool + 1;
        dbg("look! i put them on the map! right? right??? right??????");
...
static void legend_doodle_set(char kind, char *str) {
  ...
  dbgf("im about to start scribbling at %lu + %d\n", state->render->legend, index);
